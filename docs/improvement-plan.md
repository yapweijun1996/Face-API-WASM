...
**Deferred + human-in-the-loop workflow (added 2026-06-14).** Liveness now supports two timings (setting "Verification timing", default **deferred**): *realtime* (blocks clock-in ~seconds) vs *deferred* (capture frames → clock in instantly → HR batch-verifies later). Frames live in a separate IndexedDB store (`attendance_frames`) so `getAllAttendance` stays light at scale. Record states: `pending → real | suspect | error → error_permanent` (after `verifyMaxAttempts`, default 3), plus `reviewed`. HR tools on the Records tab: batch verify (scope all/today/week, progress bar + ETA, interruptible/resumable), multi-status filter chips + completion summary, human review modal (mark-live / delete-recapture for `error_permanent`, incl. bulk), retry-failed, and **auto-retry** (edge-triggered on LM Studio health recovery; paused while tab hidden). See `examples/TMS/README.md` → "延迟核验与 HR 复核工作流".
...
**Status:** ✅ Implemented v1 in `examples/TMS/js/tms-geometry.js` — passive signal only, must be calibrated on real live / phone / print samples before stricter enforcement.
...
**Implemented v1 for this repo:**
- Uses existing `face_landmark_68` points; no new action challenge.
- Tracks passive landmarks per face track for about 2.5 seconds / 12 samples.
- Computes yaw proxy, nose/mouth parallax proxy, and a lightweight planar-motion suspect score.
- Stores `geometryScore`, `geometrySuspect`, and `geometryReason` on attendance records.
- Treats geometry as `suspect` metadata for deferred HR/VLM review, not as a certified PAD hard block.
...
**Status:** ✅ Implemented v1 in `examples/TMS/js/tms-liveness.js`; validate on local real attacks before tightening thresholds.
...
**Output contract:** Strict JSON includes explicit spoof-cue fields so records are auditable:
```json
{
  "real": false,
  "confidence": 0.18,
...
**2026-06-14 hardening after false-verified phone sample:** The prompt now uses `VLM_PROMPT_VERSION = "vlm-spoof-cues-v2"` and explicitly says any visible phone/screen in any frame must be `real=false` with `attack_type="phone_screen"`. Send frames at 768px / JPEG 0.78 so small phones, bezels, glare, and moire are less likely to disappear during compression. Attendance records store `verifyVersion`, and batch verification re-runs old-version `real/suspect` records so previous false `Verified` results can be corrected by newer rules.

**2026-06-14 review-mode update:** TMS now captures configurable full-frame samples: 3-6 images, 0.5-3s interval, default 6 images at 0.5s. End users can choose the VLM review mode in Settings: `batch6` sends all selected frames in one request, `session6` checks one frame at a time while carrying forward prior verdict history, and `single6` checks each frame independently. The aggregate rule is strict: if any frame is spoof/uncertain, the record becomes `suspect`. Attendance records store `verifyReviewMode`, `verifyFrameCount`, `verifyFrameTargetCount`, and `verifyFrameIntervalMs`; batch verification re-runs old `real/suspect` records when the selected review mode or sampling settings change.
...
| P3-4 Passive 3D-vs-planar landmark consistency | P3 | Research + calibration | ✅ Implemented v1 as passive signal, not certified PAD |
| P3-5 Full-frame VLM spoof-cue prompt hardening | P3 | Low/medium | ✅ Implemented v1 with JSON spoof cues + uncertainty |
