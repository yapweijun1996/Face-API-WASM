...
| 标签 | 能力 |
|------|------|
| **打卡** | 实时摄像头 **多人同时**识别（detectAllFaces）。每张脸**独立**画出姓名标签 + 倒计时环、独立活体检测、独立自动打卡——一队人可以挨个走过依次打卡。识别到员工后**对准保持 ~1.5 秒自动打卡**，无需按钮；成功后从该人脸位置**迸发庆祝粒子** + **语音问候** + 提示音。**活体检测**：打卡前必须眨一次眼，照片/截图骗不过。60 秒冷却防误触。 |
...
- 👥 **多人同时打卡**：一次识别画面里所有人脸，每个人独立倒计时/活体/打卡，适合上班高峰排队。
- 🎉 **打卡成功粒子庆祝**：从人脸位置迸发彩色粒子（上班绿、下班红）。
- 🎥 **实时画面标签 + 自动打卡**：姓名/倒计时环直接叠加在视频上，对准即打卡，免按钮。
- 👁 **活体检测（眨眼防伪）**：基于 68 个人脸关键点算眼睛纵横比（EAR），要求眨眼，挡住照片攻击。
