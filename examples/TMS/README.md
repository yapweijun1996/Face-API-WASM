...
| 标签 | 能力 |
|------|------|
| **打卡** | 实时摄像头 **多人同时**识别（detectAllFaces）。用 **IoU 人脸追踪**把每帧人脸框关联到稳定 track，**按位置**（而非按员工 id）维护各自的倒计时 / 真人检测 / 自动打卡——同一员工被两张脸（本人 + 墙上照片 / 双胞胎）同时拍到也互不串状态，且经冷却去重只写一次。识别到员工后**对准保持 ~1.5 秒自动打卡**，无需按钮；成功后从该人脸位置**迸发庆祝粒子** + **语音问候** + 提示音。**ONNX 真人检测（可选，默认关闭）**：在「记录 → 防伪」开启后，系统会用本地 MiniFASNet-V2 判断人脸是真人、打印照片还是屏幕回放。60 秒冷却防误触。 |
...
- **ONNX 真人检测（可选，默认关闭）**：基于本地 MiniFASNet-V2 anti-spoofing 模型，对匹配成功的人脸 crop 输出 live / print-attack / replay-attack 概率；在设置页「防伪」一键开关。
